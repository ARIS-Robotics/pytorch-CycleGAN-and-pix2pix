...
class RetinaGANModel(CycleGANModel):
    ...
    a '--netD retinaGAN' discriminator (based on PatchGAN),
    and a least-square GANs objective ('--gan_mode lsgan').

    RetinaGAN paper: https://arxiv.org/pdf/2011.03148.pdf
    """
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        For RetinaGAN, in addition to GAN losses, we introduce lambda_A, lambda_B, and lambda_prcp for the following losses.
        A (source domain), B (target domain).
        Generators: G_A: A -> B; G_B: B -> A. Denoted as "G" and "F" respectively in the original paper
        Discriminators: D_A: G_A(A) vs. B; D_B: G_B(B) vs. A.
        Forward cycle loss:  lambda_A * ||G_B(G_A(A)) - A|| (Eqn. (2) in CycleGAN paper)
        Backward cycle loss: lambda_B * ||G_A(G_B(B)) - B|| (Eqn. (2) in CycleGAN paper)
